...
<div>
                ...
                <div class="file_link selected" aria-hidden="true" data-toggle="tooltip" data-placement="auto" :title="file">
                    <div class="link_icon">
                        <template v-if="fileIs(file, 'image')">
                            <div class="img_icon" :style="imgIcon('{{ Storage::disk(config('admin.storage.disk'))->url('/') }}'+file)"></div>
                        </template>
                    ...
                    <i class="admin-x dd-nodrag" v-on:click="removeFileFromInput(file)"></i>
                </div>
            </li>
        </ol>
    </div>
    <div v-if="hidden_element">
        <div class="btn btn-sm btn-default" v-on:click="isExpanded = !isExpanded;" style="width:100%">
            <div v-if="!isExpanded"><i class="admin-double-down"></i> {{ __('admin::generic.open') }}</div>
            <div v-if="isExpanded"><i class="admin-double-up"></i> {{ __('admin::generic.close') }}</div>
        </div>
    </div>
    <div id="toolbar" v-if="showToolbar" :style="isExpanded ? 'display:block' : 'display:none'">
        ...
        <div class="btn-group offset-right">
            <button type="button" :disabled="selected_files.length == 0" v-if="allowUpload && hidden_element" class="btn btn-default" v-on:click="addSelectedFiles()">
                <i class="admin-upload"></i>
                {{ __('admin::media.add_all_selected') }}
            </button>
            <button type="button" v-if="showFolders && allowMove" class="btn btn-default" data-toggle="modal" :data-target="'#move_files_modal_'+this._uid">
                <i class="admin-move"></i>
                {{ __('admin::generic.move') }}
            </button>
        ...
        <div class="breadcrumb-container">
            <ol class="breadcrumb filemanager">
                <li class="media_breadcrumb" v-on:click="setCurrentPath(-1)">
                    ...
                    <strong>{{ __('admin::media.library') }}</strong>
                </li>
            ...
            <div id="right">
                <div class="right_details">
                    <div v-if="selected_files.length > 1" class="right_none_selected">
                        <i class="admin-list"></i>
                        <p>@{{ selected_files.length }} {{ __('admin::media.files_selected') }}</p>
                    </div>
                        ...
                        <div class="detail_img">
                            ...
                            <div v-else-if="fileIs(selected_file, 'video')">
                                <video width="100%" height="auto" ref="videoplayer" controls>
                                    ...
                                    {{ __('admin::media.browser_video_support') }}
                                </video>
                            </div>
                            <div v-else-if="fileIs(selected_file, 'audio')">
                                ...
                                <audio controls style="width:100%; margin-top:5px;" ref="audioplayer">
                                    ...
                                    {{ __('admin::media.browser_audio_support') }}
                                </audio>
                            </div>
                            ...
                            <div v-else>
                                <i class="admin-file-text"></i>
                            </div>
                        </div>
                        <div class="detail_info">
                            <span>
                                <h4>{{ __('admin::media.title') }}:</h4>
                            ...
                            <span>
                                <h4>{{ __('admin::media.type') }}:</h4>
                                <p>@{{ selected_file.type }}</p>
                            </span>

                            <template v-if="!fileIs(selected_file, 'folder')">
                                <span>
                                    <h4>{{ __('admin::media.size') }}:</h4>
                                    <p><span class="selected_file_size">@{{ bytesToSize(selected_file.size) }}</span></p>
                                </span>
                                <span>
                                    <h4>{{ __('admin::media.public_url') }}:</h4>
                                    <p><a :href="selected_file.path" target="_blank">{{ __('admin::generic.click_here') }}</a></p>
                                </span>
                                <span>
                                    <h4>{{ __('admin::media.last_modified') }}:</h4>
    ...
    <div class="modal fade modal-info" :id="'create_dir_modal_'+this._uid">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <input name="new_folder_name" placeholder="{{ __('admin::media.new_folder_name') }}" class="form-control" value="" v-model="modals.new_folder.name" />
                </div>

                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">{{ __('admin::generic.cancel') }}</button>
                    <button type="button" class="btn btn-info" v-on:click="createFolder">{{ __('admin::media.create_new_folder') }}
                    </button>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade modal-danger" :id="'confirm_delete_modal_'+this._uid" v-if="allowDelete">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <h4>{{ __('admin::media.delete_question') }}</h4>
                    <ul>
                        <li v-for="file in selected_files">@{{ file.name }}</li>
                    </ul>
                    <h5 class="folder_warning">
                        <i class="admin-warning"></i> {{ __('admin::media.delete_folder_question') }}
                    </h5>
                </div>

                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">{{ __('admin::generic.cancel') }}</button>
                    <button type="button" class="btn btn-danger" v-on:click="deleteFiles">{{ __('admin::generic.delete_confirm') }}
                    </button>
                </div>
            </div>
        </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <select class="form-control" v-model="modals.move_files.destination">
                        <option value="" disabled>{{ __('admin::media.destination_folder') }}</option>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">{{ __('admin::generic.cancel') }}</button>
                    <button type="button" class="btn btn-warning" v-on:click="moveFiles">{{ __('admin::generic.move') }}</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade modal-warning" :id="'crop_modal_'+this._uid" v-if="allowCrop">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    <h4 class="modal-title">{{ __('admin::media.crop_image') }}</h4>
                </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="new-image-info">
                        {{ __('admin::media.width') }} <span :id="'new-image-width_'+this._uid"></span>, {{ __('admin::media.height') }}<span :id="'new-image-height_'+this._uid"></span>
                    </div>
                </div>

                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">{{ __('admin::generic.cancel') }}</button>
                    <button type="button" class="btn btn-warning" v-on:click="crop(false)">{{ __('admin::media.crop') }}</button>
                    <button type="button" class="btn btn-warning" v-on:click="crop(true)">{{ __('admin::media.crop_and_create') }}</button>
                </div>
            </div>
        </div>
    </div>
        ...
        methods: {
                ...
                if (file.type != 'folder') {
                    if (!this.allowMultiSelect) {
                        ...
                        if (content.length >= this.maxSelectedFiles && this.maxSelectedFiles > 0) {
                            var msg_sing = "{{ trans_choice('admin::media.max_files_select', 1) }}";
                            var msg_plur = "{{ trans_choice('admin::media.max_files_select', 2) }}";
            ...
            renameFile: function(object) {
                ...
                $.post('{{ route('admin.media.rename') }}', {
					...
					if (data.success == true) {
						toastr.success('{{ __('admin::media.success_renamed') }}', "{{ __('admin::generic.sweet_success') }}");
						vm.getFiles();
					} else {
						toastr.error(data.error, "{{ __('admin::generic.whoopsie') }}");
					}
				});
            },
            createFolder: function(e) {
                ...
                $.post('{{ route('admin.media.new_folder') }}', { new_folder: vm.current_folder+'/'+name, _token: '{{ csrf_token() }}' }, function(data) {
					if(data.success == true){
						toastr.success('{{ __('admin::generic.successfully_created') }} ' + name, "{{ __('admin::generic.sweet_success') }}");
						vm.getFiles();
					} else {
						toastr.error(data.error, "{{ __('admin::generic.whoopsie') }}");
					}
            ...
            deleteFiles: function() {
                ...
                $.post('{{ route('admin.media.delete') }}', {
					...
					if(data.success == true){
						toastr.success('', "{{ __('admin::generic.sweet_success') }}");
						vm.getFiles();
            ...
            moveFiles: function(e) {
				...
				$.post('{{ route('admin.media.move') }}', {
					...
					if(data.success == true){
						toastr.success('{{ __('admin::media.success_moved') }}', "{{ __('admin::generic.sweet_success') }}");
						vm.getFiles();
					} else {
						toastr.error(data.error, "{{ __('admin::generic.whoopsie') }}");
					}
            ...
            crop: function(mode) {
                ...
                if (!mode) {
                    if (!window.confirm('{{ __('admin::media.crop_override_confirm') }}')) {
						return;
					}
                }
            ...
            if (this.allowUpload && !dropzone.hasClass('dz-clickable')) {
                dropzone.dropzone({
                    ...
                    success: function(e, res) {
                        if (res.success) {
                            toastr.success(res.message, "{{ __('admin::generic.sweet_success') }}");
                        } else {
                            toastr.error(res.message, "{{ __('admin::generic.whoopsie') }}");
                        }
                    },
                    error: function(e, res, xhr) {
                        toastr.error(res, "{{ __('admin::generic.whoopsie') }}");
                    },
                    ...
                    if (vm.hidden_element) {
                        if (vm.maxSelectedFiles > 1) {
                            ...
                            if (content.length < vm.minSelectedFiles) {
                                e.preventDefault();
                                var msg_sing = "{{ trans_choice('admin::media.min_files_select', 1) }}";
                                var msg_plur = "{{ trans_choice('admin::media.min_files_select', 2) }}";
                            ...
                            if (vm.minSelectedFiles > 0 && vm.hidden_element.value == '') {
                                e.preventDefault();
                                toastr.error("{{ trans_choice('admin::media.min_files_select', 1) }}");
                            }
                        }
                    }
                });
