...
@section('page_header')
    <h1 class="page-title">
        <i class="admin-list-add"></i> {{ $dataType->getTranslatedAttribute('display_name_plural') }}
                    ...
                    <div class="panel-body">
                        <table id="dataTable" class="table table-hover">
                            ...
                            <tr>
                                ...
                                @endforeach
                                <th class="actions text-right">{{ __('admin::generic.actions') }}</th>
                            </tr>
                            </thead>
                            <tbody>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">
                        <i class="admin-trash"></i> {{ __('admin::generic.delete_question') }} {{ $dataType->getTranslatedAttribute('display_name_singular') }}?
                    </h4>
                </div>
                <div class="modal-footer">
                    <form action="#" id="delete_form" method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
                        <input type="submit" class="btn btn-danger pull-right delete-confirm" value="{{ __('admin::generic.delete_this_confirm') }} {{ $dataType->getTranslatedAttribute('display_name_singular') }}">
                    </form>
...
@section('javascript')
    ...
    <script>
        ...
        $('td').on('click', '.delete', function (e) {
            $('#delete_form')[0].action = '{{ route('admin.'.$dataType->slug.'.destroy', ['id' => '__menu']) }}'.replace('__menu', $(this).data('id'));
