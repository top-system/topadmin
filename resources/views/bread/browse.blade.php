...
@section('page_title', __('admin::generic.viewing').' '.$dataType->getTranslatedAttribute('display_name_plural'))
        ...
        @can('edit', app($dataType->model_name))
            @if(!empty($dataType->order_column) && !empty($dataType->order_display_column))
                <a href="{{ route('admin.'.$dataType->slug.'.order') }}" class="btn btn-primary btn-add-new">
                    <i class="admin-list"></i> <span>{{ __('admin::bread.order') }}</span>
                </a>
            @endif
        @endcan
        @can('delete', app($dataType->model_name))
            @if($usesSoftDeletes)
                <input type="checkbox" @if ($showSoftDeleted) checked @endif id="show_soft_deletes" data-toggle="toggle" data-on="{{ __('admin::bread.soft_deletes_off') }}" data-off="{{ __('admin::bread.soft_deletes_on') }}">
            @endif
        @endcan
        @foreach($actions as $action)
            @if (method_exists($action, 'massAction'))
                @include('admin::bread.partials.actions', ['action' => $action, 'data' => null])
            @endif
        @endforeach
                                ...
                                <div id="search-input">
                                    ...
                                    <div class="col-2">
                                        <select id="filter" name="filter">
                                            <option value="contains" @if($search->filter == "contains") selected @endif>{{ __('admin::generic.contains') }}</option>
                                            <option value="equals" @if($search->filter == "equals") selected @endif>=</option>
                                        </select>
                                    </div>
                                    <div class="input-group col-md-12">
                                        <input type="text" class="form-control" placeholder="{{ __('admin::generic.search') }}" name="s" value="{{ $search->value }}">
                            ...
                            <table id="dataTable" class="table table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        @endforeach
                                        <th class="actions text-right dt-not-orderable">{{ __('admin::generic.actions') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            ...
                                            <td>
                                                ...
                                                @elseif($row->type == 'relationship')
                                                    @include('admin::formfields.relationship', ['view' => 'browse','options' => $row->details])
                                                ...
                                                @elseif($row->type == 'multiple_images')
                                                    ...
                                                    @if($images)
                                                        ...
                                                        @foreach($images as $image)
                                                            <img src="@if( !filter_var($image, FILTER_VALIDATE_URL)){{ Admin::image( $image ) }}@else{{ $image }}@endif" style="width:50px">
                                                        @endforeach
                                                    @endif
                                                @elseif($row->type == 'media_picker')
                                                    ...
                                                    @if ($files)
                                                        @if (property_exists($row->details, 'show_as_images') && $row->details->show_as_images)
                                                            ...
                                                            <img src="@if( !filter_var($file, FILTER_VALIDATE_URL)){{ Admin::image( $file ) }}@else{{ $file }}@endif" style="width:50px">
                                                            @endforeach
                                                        @else
                                                            <ul>
                                                            @foreach (array_slice($files, 0, 3) as $file)
                                                        ...
                                                        @endif
                                                        @if (count($files) > 3)
                                                            {{ __('admin::media.files_more', ['count' => (count($files) - 3)]) }}
                                                        @endif
                                                    @elseif (is_array($files) && count($files) == 0)
                                                        {{ trans_choice('admin::media.files', 0) }}
                                                    @elseif ($data->{$row->field} != '')
                                                        @if (property_exists($row->details, 'show_as_images') && $row->details->show_as_images)
                                                            <img src="@if( !filter_var($data->{$row->field}, FILTER_VALIDATE_URL)){{ Admin::image( $data->{$row->field} ) }}@else{{ $data->{$row->field} }}@endif" style="width:50px">
                                                        @else
                                                            {{ $data->{$row->field} }}
                                                        @endif
                                                    @else
                                        ...
                                        <td class="no-sort no-click bread-actions">
                                            @foreach($actions as $action)
                                                @if (!method_exists($action, 'massAction'))
                                                    @include('admin::bread.partials.actions', ['action' => $action])
                                                @endif
                                            @endforeach
                                        </td>
                                    </tr>
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form action="#" id="delete_form" method="POST">
                        {{ method_field('DELETE') }}
                        {{ csrf_field() }}
                        <input type="submit" class="btn btn-danger pull-right delete-confirm" value="{{ __('admin::generic.delete_confirm') }}">
                    </form>
...
@section('javascript')
    ...
    <script>
        ...
        $('td').on('click', '.delete', function (e) {
            $('#delete_form')[0].action = '{{ route('admin.'.$dataType->slug.'.destroy', '__id') }}'.replace('__id', $(this).data('id'));
            ...
            $(function() {
                $('#show_soft_deletes').change(function() {
                    if ($(this).prop('checked')) {
                        $('#dataTable').before('<a id="redir" href="{{ (route('admin.'.$dataType->slug.'.index', array_merge($params, ['showSoftDeleted' => 1]), true)) }}"></a>');
                    }else{
                        $('#dataTable').before('<a id="redir" href="{{ (route('admin.'.$dataType->slug.'.index', array_merge($params, ['showSoftDeleted' => 0]), true)) }}"></a>');
                    }
