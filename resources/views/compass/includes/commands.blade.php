@if($artisan_output)
    <pre>
        <i class="close-output admin-x">{{ __('admin::compass.commands.clear_output') }}</i><span class="art_out">{{ __('admin::compass.commands.command_output') }}:</span>{{ trim(trim($artisan_output,'"')) }}
    </pre>
@endif
...
@foreach($commands as $command)
    <div class="command" data-command="{{ $command->name }}">
        ...
        <form action="{{ route('admin.compass.post') }}" class="cmd_form" method="POST">
            {{ csrf_field() }}
            <input type="text" name="args" autofocus class="form-control" placeholder="{{ __('admin::compass.commands.additional_args') }}">
            <input type="submit" class="btn btn-primary pull-right delete-confirm"
                    value="{{ __('admin::compass.commands.run_command') }}">
