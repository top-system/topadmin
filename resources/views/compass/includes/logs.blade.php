...
<div class="col-sm-3 col-md-3 sidebar">
  <h3><i class="admin-logbook"></i> {{ __('admin::compass.logs.title') }} <small>{{ __('admin::compass.logs.text') }}.</small></h3>
...
<div class="col-sm-9 col-md-9 table-container">
  @if ($logs === null)
    <div>
      {{ __('admin::compass.logs.file_too_big') }}
    </div>
  @else
    <table id="table-log" class="table table-striped">
      <thead>
      <tr>
        <th>{{ __('admin::compass.logs.level') }}</th>
        <th>{{ __('admin::compass.logs.context') }}</th>
        <th>{{ __('admin::compass.logs.date') }}</th>
        <th>{{ __('admin::compass.logs.content') }}</th>
      </tr>
      </thead>
      <tbody>
  ...
  <div>
    @if($current_file)
      <a href="?download={{ base64_encode($current_file) }}"><span class="glyphicon glyphicon-download-alt"></span>
        {{ __('admin::compass.logs.download_file') }}</a>
      -
      <a id="delete-log" href="?del={{ base64_encode($current_file) }}"><span
            class="glyphicon glyphicon-trash"></span> {{ __('admin::compass.logs.delete_file') }}</a>
      @if(count($files) > 1)
        -
        <a id="delete-all-log" href="?delall=true"><span class="glyphicon glyphicon-trash"></span> {{ __('admin::compass.logs.delete_all_files') }}</a>
      @endif
    @endif
  </div>
</div>
