                        ...
                        <div id="{{ \Illuminate\Support\Str::slug($group) }}" class="tab-pane fade in @if($group == $active) active @endif">
                            ...
                            <div class="panel-heading">
                                ...
                                <div class="panel-actions">
                                    ...
                                    @can('delete', Admin::model('Setting'))
                                    <i class="admin-trash"
                                       data-id="{{ $setting->id }}"
                                    ...
                                    @elseif($setting->type == "image" || $setting->type == "file")
                                        @if(isset( $setting->value ) && !empty( $setting->value ) && Storage::disk(config('admin.storage.disk'))->exists($setting->value))
                                            <div class="img_settings_container">
                                                <a href="{{ route('admin.settings.delete_value', $setting->id) }}" class="admin-x delete_value"></a>
                                        ...
                                        @elseif($setting->type == "file" && isset( $setting->value ))
                                            @if(json_decode($setting->value) !== null)
                                                @foreach(json_decode($setting->value) as $file)
                                                  <div class="fileType">
                                                    <a class="fileType" target="_blank" href="{{ Storage::disk(config('admin.storage.disk'))->url($file->download_link) }}">
                                                      {{ $file->original_name }}
                                                    </a>
                                                    <a href="{{ route('admin.settings.delete_value', $setting->id) }}" class="admin-x delete_value"></a>
                                                 </div>
        ...
        <div class="panel" style="margin-top:10px;">
            ...
            <div class="panel-body">
                <form action="{{ route('admin.settings.store') }}" method="POST">
                    ...
                    <div class="col-md-3">
                        <label for="type">{{ __('admin::generic.type') }}</label>
...
@section('javascript')
    <script>
        $('document').ready(function () {
            ...
            @can('delete', Admin::model('Setting'))
            $('.panel-actions .admin-trash').click(function () {
                var display = $(this).data('display-name') + '/' + $(this).data('display-key');

                $('#delete_setting_title').text(display);

                $('#delete_form')[0].action = '{{ route('admin.settings.delete', [ 'id' => '__id' ]) }}'.replace('__id', $(this).data('id'));
            ...
            tinymce.init(window.adminTinyMCE.getConfig());
        });
    </script>
