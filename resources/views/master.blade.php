<!DOCTYPE html>
<html lang="{{ config('app.locale') }}" dir="{{ __('admin::generic.is_rtl') == 'true' ? 'rtl' : 'ltr' }}">
<head>
    <title>@yield('page_title', setting('admin.title') . " - " . setting('admin.description'))</title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="csrf-token" content="{{ csrf_token() }}"/>
    <meta name="assets-path" content="{{ route('admin.admin_assets') }}"/>

    <!-- Google Fonts -->
    <link href="https://fonts.googleapis.com/css?family=Open+Sans:300,400,700" rel="stylesheet">

    <!-- Favicon -->
    <?php $admin_favicon = Admin::setting('admin.icon_image', ''); ?>
    @if($admin_favicon == '')
        <link rel="shortcut icon" href="{{ admin_asset('images/logo-icon.png') }}" type="image/png">
    @else
        <link rel="shortcut icon" href="{{ Admin::image($admin_favicon) }}" type="image/png">
    @endif
    ...
    @yield('css')
    @if(__('admin::generic.is_rtl') == 'true')
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-rtl/3.4.0/css/bootstrap-rtl.css">
        <link rel="stylesheet" href="{{ admin_asset('css/rtl.css') }}">
    @endif
    ...
    <div class="row content-container">
        ...
        <script>
            (function(){
                    ...
                    if (window.innerWidth > 768 && window.localStorage && window.localStorage['admin.stickySidebar'] == 'true') {
