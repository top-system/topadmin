...
use TopSystem\TopAdmin\Events\MenuDisplay;
...
class Menu extends Model
{
    ...
    public static function display($menuName, $type = null, array $options = [])
    {
        ...
        $menu = \Cache::remember('admin_menu_'.$menuName, \Carbon\Carbon::now()->addDays(30), function () use ($menuName) {
    ...
    protected static function processItems($items)
    {
        ...
        $items = $items->transform(function ($item) {
            ...
            if (($item->href == url('') || $item->href == route('admin.dashboard')) && $item->children->count() > 0) {
                // Exclude sub-menus
                $item->active = false;
            } elseif ($item->href == route('admin.dashboard') && url()->current() != route('admin.dashboard')) {
                // Exclude dashboard
                $item->active = false;
            }
