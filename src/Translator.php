...
class Translator implements ArrayAccess, JsonSerializable
{
    ...
    public function save()
    {
        ...
        foreach ($attributes as $key => $attribute) {
            if ($attribute['exists']) {
                ...
                $translation = AdminFacade::model('Translation')->where('table_name', $this->model->getTable())
            ...
            if (is_null($translation)) {
                $translation = AdminFacade::model('Translation');
            }
    ...
    public function deleteTranslation($key)
    {
        ...
        AdminFacade::model('Translation')->where('table_name', $this->model->getTable())
            ->where('column_name', $key)
            ->where('foreign_key', $this->model->getKey())
            ->where('locale', $locale)
            ->delete();
    ...
    public function __call($method, array $arguments)
    {
        if (!$this->model->hasTranslatorMethod($method)) {
            throw new \Exception('Call to undefined method TopSystem\TopAdmin\Translator::'.$method.'()');
        }
