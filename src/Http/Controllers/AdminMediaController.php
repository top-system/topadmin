...
use TopSystem\TopAdmin\Events\MediaFileAdded;
...
class AdminMediaController extends Controller
{
    ...
    public function files(Request $request)
    {
        ...
        foreach ($storageItems as $item) {
            if ($item['type'] == 'dir') {
                ...
                if (empty(pathinfo($item['path'], PATHINFO_FILENAME)) && !config('admin.hidden_files')) {
                    continue;
                }
    ...
    public function rename(Request $request)
    {
        ...
        if (pathinfo($filename)['extension'] !== pathinfo($newFilename)['extension']) {
            $error = __('admin::media.error_renaming_ext');
        } else {
    ...
    public function upload(Request $request)
    {
        ...
        try {
            ...
            $allowedMimeTypes = config('admin.media.allowed_mimetypes', '*');
            if ($allowedMimeTypes != '*' && (is_array($allowedMimeTypes) && !in_array($request->file->getMimeType(), $allowedMimeTypes))) {
                throw new Exception(__('admin::generic.mimetype_not_allowed'));
            }
