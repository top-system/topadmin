...
class AdminCompassController extends Controller
{
    protected $request;
    ...
    public function index(Request $request)
    {
        ...
        if (!\App::environment('local') && !config('admin.compass_in_production', false)) {
        ...
        if ($this->request->input('download')) {
            ...
            return redirect($this->request->url().'?logs=true')->with([
                'message'    => __('admin::compass.logs.delete_success').' '.base64_decode($this->request->input('del')),
                'alert-type' => 'success',
            ]);
            ...
            return redirect($this->request->url().'?logs=true')->with([
                'message'    => __('admin::compass.logs.delete_all_success'),
                'alert-type' => 'success',
            ]);
        }
