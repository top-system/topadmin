...
use TopSystem\TopAdmin\Events\FileDeleted;
use TopSystem\TopAdmin\Http\Controllers\ContentTypes\Checkbox;
use TopSystem\TopAdmin\Http\Controllers\ContentTypes\Coordinates;
use TopSystem\TopAdmin\Http\Controllers\ContentTypes\File;
use TopSystem\TopAdmin\Http\Controllers\ContentTypes\Image as ContentImage;
use TopSystem\TopAdmin\Http\Controllers\ContentTypes\MultipleCheckbox;
use TopSystem\TopAdmin\Http\Controllers\ContentTypes\MultipleImage;
use TopSystem\TopAdmin\Http\Controllers\ContentTypes\Password;
use TopSystem\TopAdmin\Http\Controllers\ContentTypes\Relationship;
use TopSystem\TopAdmin\Http\Controllers\ContentTypes\SelectMultiple;
use TopSystem\TopAdmin\Http\Controllers\ContentTypes\Text;
use TopSystem\TopAdmin\Http\Controllers\ContentTypes\Timestamp;
use TopSystem\TopAdmin\Traits\AlertsMessages;
use Validator;
...
abstract class Controller extends BaseController
{
    ...
    public function insertUpdateData($request, $slug, $rows, $data)
    {
        ...
        if ($request->session()->has($slug.'_path') || $request->session()->has($slug.'_uuid')) {
            ...
            if ($old_path != $new_path && 
                !Storage::disk(config('admin.storage.disk'))->exists($new_path) &&
                Storage::disk(config('admin.storage.disk'))->exists($old_path)
                ) 
            {
                $request->session()->forget([$slug.'_path', $slug.'_uuid']);
                Storage::disk(config('admin.storage.disk'))->move($old_path, $new_path);
                Storage::disk(config('admin.storage.disk'))->deleteDirectory($folder_path);
            }
        }
    ...
    {
        if ($path && Storage::disk(config('admin.storage.disk'))->exists($path)) {
