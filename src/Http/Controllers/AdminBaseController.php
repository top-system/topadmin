...
use TopSystem\TopAdmin\Database\Schema\SchemaManager;
use TopSystem\TopAdmin\Events\BreadDataAdded;
use TopSystem\TopAdmin\Events\BreadDataDeleted;
use TopSystem\TopAdmin\Events\BreadDataRestored;
use TopSystem\TopAdmin\Events\BreadDataUpdated;
use TopSystem\TopAdmin\Events\BreadImagesDeleted;
use TopSystem\TopAdmin\Facades\Admin;
use TopSystem\TopAdmin\Http\Controllers\Traits\BreadRelationshipParser;
...
class AdminBaseController extends Controller
{
    ...
    public function show(Request $request, $id)
    {
        ...
        return Admin::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (auth()->user()->can('browse', app($dataType->model_name))) {
            $redirect = redirect()->route("admin.{$dataType->slug}.index");
        } else {
    ...
    public function restore(Request $request, $id)
    {
        ...
        $data = $res
            ? [
                'message'    => __('admin::generic.successfully_restored')." {$displayName}",
                'alert-type' => 'success',
            ]
            : [
                'message'    => __('admin::generic.error_restoring')." {$displayName}",
                'alert-type' => 'error',
            ];
        ...
        try {
            ...
            if (@json_decode($multi)) {
                ...
                if (is_null($key) || $key === false) {
                    throw new Exception(__('admin::media.file_does_not_exist'), 400);
                }
            ...
            return response()->json([
                'data' => [
                    'status'  => 200,
                    'message' => __('admin::media.file_removed'),
                ],
            ]);
        } catch (Exception $e) {
            $code = 500;
    ...
    public function deleteBreadImages($data, $rows, $single_image = null)
    {
        ...
        foreach ($rows as $row) {
            ...
            foreach ($images_to_remove as $image) {
                ...
                if ($image != config('admin.user.default_avatar') && (is_null($single_image) || $single_image == $image)) {
    ...
    public function order(Request $request)
    {
        ...
        if (empty($dataType->order_column) || empty($dataType->order_display_column)) {
            ...
            ->with([
                'message'    => __('admin::bread.ordering_not_set'),
                'alert-type' => 'error',
            ]);
        }
        ...
        $dataRow = Admin::model('DataRow')->whereDataTypeId($dataType->id)->whereField($display_column)->first();

        $view = 'admin::bread.order';

        if (view()->exists("admin::$slug.order")) {
            $view = "admin::$slug.order";
        }
