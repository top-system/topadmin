...
class MakeControllersCommand extends Command
{
    ...
    {
        $stub = $this->getStub();
        $files = $this->filesystem->files(base_path('vendor/topsystem/topadmin/src/Http/Controllers'));
        $namespace = config('admin.controllers.namespace', 'TopSystem\\TopAdmin\\Http\\Controllers');
    ...
    {
        return $this->filesystem->get(base_path('/vendor/TopSystem/TopAdmin/stubs/'.$this->stub));
    }
    ...
    protected function generateContent($stub, $class)
    {
        ...
        $content = str_replace(
            'FullBaseDummyClass',
            'TopSystem\\TopAdmin\\Http\\Controllers\\'.$class,
            $content
        );
